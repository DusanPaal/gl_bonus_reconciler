...
if __name__ == "__main__":
	...
	parser.add_argument("-a", "--action", required = False, help = "Action to perform.", choices = ["reconcile", "benchmark"])
	parser.add_argument("-d", "--debug", required = False, help = "Runs app in debug mode.")
	parser.add_argument("-e", "--email_id", required = False, help = "Sender email id.")
	parser.add_argument("-t", "--database", required = False, help = "Database to use as the data source.", choices = ["remote", "local"], default = "remote")
	ret_code = main(vars(parser.parse_args()))

	_logger.info(f"=== System shutdown with return code: {ret_code} ===")
	logging.shutdown()
	sys.exit(ret_code)
